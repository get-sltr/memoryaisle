...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      return new Response(
        JSON.stringify({ success: false, intent: 'error', items: [], response: 'Not authenticated' }),
    ...
    const { data: quotaResult } = await serviceClient.rpc('increment_daily_usage', {
      p_user_id: user.id,
      p_feature: 'mira_queries',
    });
    if (quotaResult === -1) {
      return new Response(
        JSON.stringify({ success: false, intent: 'error', items: [], response: "You've reached your daily limit. Upgrade to Premium for unlimited access!" }),
