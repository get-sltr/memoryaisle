...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      return new Response(
        JSON.stringify({ success: false, items: [], message: 'Not authenticated' }),
    ...
    if (quotaResult === -1) {
      return new Response(
        JSON.stringify({ success: false, items: [], message: "You've reached your daily limit. Upgrade to Premium for unlimited!" }),
